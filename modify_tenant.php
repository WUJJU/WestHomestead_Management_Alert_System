  ...
  <div id="main">
    ...
    <div id="site_content">
      ...
      <div id="content">
       ...
       <h3>Search</h3>
        <form method="get" action="modify_tenant.php" id="search_form">
          <p>
      ...
      <?php
       ...
       $sql1="SELECT tenant_id from live where parcel_id='{$pid}' ";
         ...
         while($tid=mysqli_fetch_array($result1,MYSQLI_ASSOC)){
           // $_SESSION['t_id']=$tid['tenant_id'];
        $sql2="SELECT * from  tenant where tid = {$tid['tenant_id']} ";
          $result2=mysqli_query($conn,$sql2);

         echo "<form action='do_sql4.php' method='post'><input type='hidden' name='tid_ea' value='{$tid['tenant_id']}' /><table><tr><th>Name on Lease</th><th>Cellphone</th><th>email</th><th>Additional Member</th></tr>";
          while( $row=mysqli_fetch_array($result2,MYSQLI_NUM)){
              echo "<tr><td><input type='text' name='name_lease' value='{$row[1]}' /></td><td><input type='text' name='cellphone' value='{$row[2]}' /></td><td><input type='text' name='email' value='{$row[3]}' /></td><td><input type='text' name='add_mem' value='{$row[4]}'/></td>";
         ...
         echo "<table><tr><th>Name</th><th>Under 18?</th></tr>";
          while( $row1=mysqli_fetch_array($result3,MYSQLI_NUM)){
              echo "<tr><td><input type='text' name='add_name[]' value='{$row1[1]}' /></td><td><input type='text' name='under18[]' value='{$row1[2]}' /></td>";
          }
          echo "</tr></table>";
          }
         ...
         //$_SESSION['t_id']=$tid;
          ...
          $result2=mysqli_query($conn,$sql2);
                echo "<form action='do_sql4.php' method='post'><input type='hidden' name='tid_ea' value='{$tid}' /><table><tr><th>Name on Lease</th><th>Cellphone</th><th>email</th><th>Additional Member</th></tr>";
